...
- **Release checklist omits DB-specific safety checks.** `README.md:281-296` includes lint/typecheck/test/build/smoke tests but does not include migration dry-run, backup/snapshot verification, or readiness check. (The city normalization guard this line originally also flagged as missing now exists — see the correction below.)
...
- README release checklist is useful but incomplete. It lists lint/typecheck/test/build and smoke tests at `README.md:281-296`, but omits DB migration dry-run, backup/snapshot, readiness endpoint, and rollback checks that are present in `docs/deployment.md:59-85` and `docs/deployment.md:142-164`.
  - **Correction:** an earlier version of this line also listed `npm run check:city-normalization`. That script did not exist in `backend/package.json` and never had a CI step — see the stale-claim note at the top of `EXECUTION_CLOSURE_REPORT_2026-03-30.md` and the note in `deployment.md`. Removed here so the three documents agreed.
  - **Update (2026-08-14):** it exists now — `backend/scripts/check_city_normalization.ts`, `npm run check:city-normalization`, wired into `.github/workflows/ci.yml` gated on the `DATABASE_URL` secret. Run against production the day it was added, it found 13 rows whose `city_normalized` does not resolve to a real Maharashtra district (9 college names leaked into `city` during parsing, one orphaned taluka, one inconsistent duplicate, one NULL) — none fixed yet, since correcting the underlying data is a separate, deliberate action from adding the check.
