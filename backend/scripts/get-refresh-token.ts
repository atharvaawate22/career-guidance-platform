...
async function getRefreshToken() {
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    ...
    console.error(
      '   Please follow steps 1-4 in GOOGLE_CALENDAR_SETUP.md first',
    );
    process.exit(1);
  }
  ...
  console.log(
    'Note: You will be redirected to http://localhost:5000/auth/google/callback?code=...',
  );
  ...
  rl.question('Paste the authorization code here: ', async (code) => {
    ...
    try {
      ...
      console.error(
        '2. Your redirect URI matches exactly in Google Cloud Console',
      );
