...
| `025_cutoffs_performance_indexes.sql` | Align the cutoffs indexes with the queries that run. Measured before → after: percentile→rank lookup 483 ms/69,007 buffers → 1.97 ms/11; cutoff explorer 637 ms/13,142 buffers → 83 ms/360; predictor 10.8 ms. Also adds a trigram index on `colleges.name` for the chatbot. **Destructive:** drops `idx_courses_name_trgm`, `idx_faqs_display_order` (both 0 scans) and the superseded `idx_cutoffs_predict` |
| `026_testimonial_moderation.sql` | Adds `testimonials.status` (`pending`/`approved`/`rejected`, default `pending`) so reviews no longer auto-publish behind a word list alone; grandfathers existing rows to `approved` and narrows the public RLS policy to approved rows |
| `027_cap_schedule_events.sql` | `cap_schedule_events` table — revision-tracked CAP admission-cycle timeline (registration → merit list → CAP Rounds 1–3 → post-CAP cutoff date) for the public evergreen schedule page and its admin editor. Seeded with placeholder rows (`status = 'upcoming'`, dates `NULL`) pending the official DTE schedule. Deliberately separate from `cap_schedule` (014), which keeps serving only the chatbot's round-date lookups |
